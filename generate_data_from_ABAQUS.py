...
class SpeckleDataGenerator():
    ...
    def apply_deformation(self, image, file):
        """
        ...
        # Read displacement data from CSV
        df = pd.read_csv(file)

        # Define displacement fields
        try:
            u = df['U1'].values.reshape((self.height, self.width)) # Read Horizontal displacement (stretching)
            v = df['U2'].values.reshape((self.height, self.width)) # Read Vertical displacement (shear)
        except Exception as e:
            raise ValueError(f"Error reshaping displacement fields from file {file}: {e}")
...
if __name__ == "__main__":
    # Define amout of data to load
    max_files = 160

    # Define save data path
    save_data_path = 'generated_data_from_ABAQUS_cvs'
    # Creat data path if not already exists
    os.makedirs(save_data_path, exist_ok=True)

    # Define path to the ABAQUS files
    csv_path = 'ABAQUS_Formatted_Files_512x512_160'
    # Load files
    csv_files = sorted(glob(os.path.join(csv_path, "Disp*_extracted_formatted.csv")))[:max_files] # all_files is a list


    # Define shape of the image
    image_width, image_height = 512, 512

    for idx, file in enumerate(tqdm(csv_files, desc="Generating samples")):
        dots_density = 0.025
        ...
        deformed_image, u, v = data_generator.apply_deformation(reference_image, file)
